...
class Database:
    ...
    def get_latest_evaluations(
        ...
        try:
            query = """
                SELECT e.id AS evaluation_id, e.ticket_id, e.eval_json_path,
                       t.channel, t.closed_at
                FROM evaluations e
                JOIN tickets t ON t.ticket_id = e.ticket_id
                WHERE e.is_latest = 1
            """
    ...
    def update_sla_result(self, eval_id: int, result: EvaluationResult) -> None:
        """In-place update of the SLA-derived fields on an existing evaluation (no re-insert).

        Persists the recomputed FRT/TTR status + minutes, SLA flags, aggregate score/band, and
        every metric's rating/rating_label (covers METRIC_8/10/19). Preserves published state.
        """
        sla = result.sla_status
        frt = sla.first_response_time if sla else None
        ttr = sla.resolution_time if sla else None
        with self._cursor() as cur:
            cur.execute(
                ...
                UPDATE evaluations SET
                    aggregate_score=?, performance_band=?,
            ...
            for m in result.metrics:
                cur.execute(
                    ...
                    (str(m.rating), m.rating_label, eval_id, m.metric_id),
                )
