...
--   SUPPORT_ANALYTICS / ZENDESK_QC / COMPUTE_WH  = database / schema / warehouse (edit if yours differ)
--
...
GRANT USAGE ON SCHEMA SUPPORT_ANALYTICS.ZENDESK_QC TO ROLE TICKET_EVALUATOR_READER;

-- SELECT on every current + future table and view in the schema.
GRANT SELECT ON ALL TABLES    IN SCHEMA SUPPORT_ANALYTICS.ZENDESK_QC TO ROLE TICKET_EVALUATOR_READER;
GRANT SELECT ON FUTURE TABLES IN SCHEMA SUPPORT_ANALYTICS.ZENDESK_QC TO ROLE TICKET_EVALUATOR_READER;
GRANT SELECT ON ALL VIEWS     IN SCHEMA SUPPORT_ANALYTICS.ZENDESK_QC TO ROLE TICKET_EVALUATOR_READER;
GRANT SELECT ON FUTURE VIEWS  IN SCHEMA SUPPORT_ANALYTICS.ZENDESK_QC TO ROLE TICKET_EVALUATOR_READER;
...
CREATE USER IF NOT EXISTS TICKET_EVALUATOR_READER_USER
    ...
    DEFAULT_NAMESPACE = SUPPORT_ANALYTICS.ZENDESK_QC
